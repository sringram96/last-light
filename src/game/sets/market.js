...
registerSet('market',{
 ...
 shot(p){
  ...
  if(p==='marketKeeper')return look(-1.2,y(10.2)+1.7,10.2,-3.6,y(13.4)+1.3,13.4);
 ...
 geometry(p){
  ...
  if(burst){
   ...
   lamps.push([-4.8,-2.6]);
   if(reduce||t<3.4)for(let i=0;i<4;i++){const o=hash(i,4)*1.2;box(-6+i*.7,.2,-3.9+o,-5.7+i*.7,1+hash(i,3)*.8,-3.6+o,mat('arc',6));}
  }
