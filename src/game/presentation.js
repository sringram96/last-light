...
function routeSteps(){
 ...
 if(state.theory)steps.push({vale:'Went down suspecting Vale',nell:'Went down suspecting Nell',board:'Went down suspecting the Board',none:'Went down without a theory'}[state.theory]);
...
function caseLine(){
 ...
 // The dawn interview asks who signed; the file's line must not answer it, and without proof must not lean either way.
 ...
 if(state.theory)return {vale:'THE CASE: Rook thinks Inspector Vale locked Bell under North Station.',nell:'THE CASE: Rook thinks '+(state.choice==='person'||state.note?'Nell Marrow':'the courier')+' locked Bell under North Station.',board:'THE CASE: Rook thinks the Lumen Board put Bell under North Station.',none:'THE CASE: Bell is under North Station. Rook has no theory who put him there.'}[state.theory];
 // The street deduction asks where the trail goes: the station is named only once Rook is on his way there.
...
function boardEntries(){
 ...
 if(at('result'))entries.push(state.twist?'NELL MARROW, Bell\'s apprentice: confessed to forging the work order. Cooperating witness.':nellWary()?'NELL MARROW, courier: accused by Rook at North Station, cleared by Bell. Not talking to Rook.':state.choice==='person'?'NELL MARROW, courier: cooperating witness. Says she is Bell\'s apprentice.':state.choice==='missed'?'THE COURIER: taken from Station Road by a red car with its lights off. A second missing person on Rook\'s desk.':state.note?'THE COURIER: signed a note in Bell\'s loft with the letter N, and made the maintenance call. Not yet found.':'THE COURIER: limped away toward the station with Bell\'s lantern.');
...
const endings=[
 ...
 {id:'board',name:'LIGHTS ON THE BOARD',when:()=>state.caught&&proofHeld()&&!state.stalled,closing:'Bell is alive. The lamps go dark along the canal, and this time it is only because morning has arrived. Vale is in custody, and the name above his is on the warrant in Rook\'s own hand. The station route gets its batteries back.',summary:'Vale arrested; Halden Ashe of the Lumen Board named on the paper Rook kept dry.'},
 // Home is the stay route that ends with Vale walking; an arrest in his own building that stalls falls to word against word.
 {id:'home',name:'THE LAMPLIGHTER HOME',when:()=>state.pursuit==='stay'&&!state.caught,closing:()=>'Bell is alive, and his testimony is on record. Vale walked out of his own building at dawn with Rook\'s eyes on his back. Rook brought the missing man home. '+(state.choice==='person'&&!nellWary()?'The street will be lit tomorrow because Nell knows the route.':state.choice==='missed'?'The courier who carried his lantern is still missing.':'Bell will light the street himself tomorrow.'),summary:'Rook stayed with Bell. Vale is at large; the search is a warrant now.'},
 // Closings that vary with the night are functions: the booth's words in Rook's notebook, Krane's silence, the courier taken.
 {id:'word',name:'WORD AGAINST WORD',when:()=>state.caught,closing:()=>'Bell is alive. The lamps go out along the canal because the sun is up. Vale is in custody, and it is Bell\'s word against a Night Division inspector\'s. '+(pursuing()?'Rook has read Ashe\'s name, but it is not on the warrant, and the batteries are still uptown.':'The name above Vale\'s is still a set of initials, and the batteries are still uptown.')+(state.shown?' Vale\'s own words about the order are in Rook\'s notebook. It will have to be enough.':' It will have to be enough.'),summary:'Vale arrested on Bell\'s testimony. Nothing on paper names the Board.'},
 {id:'krane',name:'THE BODYGUARD TALKS',when:()=>!state.caught&&kranePinned(),closing:()=>state.stalled?'Bell is alive. The lamps go dark along the canal; morning has come. Vale is gone, and Krane, in a splint in the room next to Vale\'s office, has stopped talking until somebody offers him a deal. The man who turned the key on Bell will name who paid for it, at a price.':'Bell is alive. The lamps go dark along the canal; morning has come. Vale is gone, but his bodyguard is in a splint in the room next to Vale\'s office, and Krane has begun to talk. The man who turned the key on Bell is naming who paid for it.',summary:'Vale escaped. Krane arrested at Substation Nine; his statement opens the Board.'},
 {id:'paper',name:'THE PAPER TRAIL',when:()=>!state.caught&&proofHeld(),closing:'Bell is alive. The lamps go dark because it is morning. Vale is at large, but the paper is dry and it names the Board. The warrant has two names on it.',summary:'Vale escaped. The signed paper names Vale and Ashe; the warrant is out.'},
 {id:'dark',name:'A VOICE IN THE DARK',when:()=>true,closing:()=>(state.choice==='missed'?'Bell is alive. The courier is not found. ':'Bell is alive. ')+'Vale remains at large, the hall is burned, and the name Rook read on the manifest burned with it. The low streets will be dark again tonight. '+(state.choice==='missed'?'One missing-person case is closed and another is open; the warrant is just beginning.':'The missing-person case is closed; the warrant is just beginning.'),summary:'Vale escaped with everything but Bell. Only Bell\'s voice remains.'}
];
