...
The menu is a set too: an idle tableau with Rook in the right third at close range (his `hero` sheet, 36 rows, the only close-up sheet in the game) and the city stacked behind him in three depths, a video billboard that cycles four pictures, vertical neon boards of ASCII stroke clusters that read as ideograms at their distance (the grid is single-width printable ASCII, so real ideograms are not drawn), two flying cars on a looping chase, and a camera that drifts for fifty seconds without a cut. Reduced motion holds its first framing.
