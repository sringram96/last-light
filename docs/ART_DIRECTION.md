...
The menu is a set too: an idle tableau with Rook in the right third at close range and the city stacked behind him in three depths. It is the one set drawn on a denser grid (`density: 2` on the set: 140 rows on desktop, the cell halved, the same field of view; the story sets stay at 70 rows and the reference frame is untouched), so Rook is drawn from his `portrait` sheet (72 rows by 50, the close-up for that grid; `hero`, 36 rows, serves the 70-row grid and phones). The city: a wet roof with coping, a ladder head, an aerial with a red lamp and a cable beading with rain; a video billboard of six pictures with chase lights round it and its colours on the wet roof; three neon boards of ASCII stroke clusters that read as ideograms at their distance (the grid is single-width printable ASCII, so real ideograms are not drawn), one flickering, one with a burnt-out cluster; lit rooms with figures, fire escapes, pipes, tanks, a crane, an LED ticker wrapped round a block, advertising panels, HOTEL down a far tower; a searchlight, an airship on a ninety-second crossing, three lanes of far traffic, warning lights climbing the megastructure, fog in bands of dimmer hue, and denser rain (`rainDensity` on the set); two flying cars on a looping chase with a third joining every third pass; and a camera that drifts for fifty-two seconds without a cut, Rook never smaller than the portrait fits. Reduced motion holds its first framing.
