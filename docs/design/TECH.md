...
6. **Colour.** Face glyphs take `faceHue`, the accent glyph its hue, everything else the actor's hue; brightness is the sheet's `level`.
...
- **Close-ups beyond the full sheet.** Above about 17 rows the full sheet is stretched more than 1.6x. Fills tile (a coat gets wider), outlines and faces stay single, blanks fill the rest. It reads as a solid figure but has no more detail than the 11x11 art; a `large` size could be added to the loader in the same shape if a close-up needs it.
