...
- **Sizes** are `portrait` (Rook's close-up for the menu's dense grid, 72 rows by 50), `hero` (the close-up for the 70-row grid; Rook's is 36 rows by 25, nobody else has one), `full`, `mid` (about 7x7) and `small` (about 5x5). Any row and column counts work; the engine reads them from the art. Every row of one size is padded to the widest row of that size, so keep all rows of a size the same length.
...
The fill rule in `resize()` (`src/game/runtime.js`) draws 70 rows on desktop and reads a set's `density` (the menu: 2) to draw `70 * density` rows in the same stage height: the cell is divided by the density, columns run up to `240 * density`, and `fx`/`fy` are scaled with the grid so the field of view is the approved one; a phone portrait scales its counts the same way. `render()` re-sizes whenever the current set's density differs from the one last sized for, so the menu and a story set swap grids as the scene changes (`cinemaAudit().grid.density`). Without a stage (the harness) the fixed rule stands at density 1, so the reference test measures what it always did. On the dense grid the frame is drawn as runs of equal ink (one `fillText` per run of up to 24 cells); story sets keep the cell-by-cell draw. Measured in Chromium at 1440x900: the menu at 480x140 renders in about 32 ms median (p90 about 36 ms) against 28 ms for the old 240x70 tableau; the frame gate is 66 ms.
